...
public class SQLiteDataAdapter implements IDataAdapter {
    ...
    public int saveCustomer(CustomerModel product) {
        try {
            String sql = "INSERT INTO Customers VALUES " + product;
            System.out.println(sql);
            Statement stmt = conn.createStatement();
            stmt.executeUpdate(sql);
    ...
    public int savePurchase(PurchaseModel purchase) {
        try {
            ...
            Statement stmt = conn.createStatement();
            stmt.executeUpdate(sql);
            CustomerModel customer = this.loadCustomer(purchase.mCustomerID);
            ProductModel product = this.loadProduct(purchase.mProductID);
            TXTReceiptBuilder receipt = new TXTReceiptBuilder();
            receipt.appendHeader("Holden Wells's Mart O Max");
            receipt.appendCustomer(customer);
            receipt.appendProduct(product);
            receipt.appendPurchase(purchase);
            receipt.appendFooter("Thank you come again!");
    ...
    public CustomerModel loadCustomer(int id) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                customer.mPayment = rs.getString("Payment");
            }
